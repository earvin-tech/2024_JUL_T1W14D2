...
# get a single products - /products/id - GET
...
def delete_product(product_id):
    ...
    if product:
        ...
        db.session.commit()
        # respond with a message saying that the product has been deleted
        return {"message": f"Product '{product.name}' deleted successfully"}
    # else
    else:
        # respond with a message saying product with thatt id does not exist
        return {"message": f"Product with id {product_id} does not exist"}, 404
...
def get_categories():
    ...
    # convert this list of python objects into a serializable format
...
def delete_category(category_id):
    ...
    if category:
        ...
        return {"message": f"Category with id {category.name} deleted successfully"}
    # else
    else:
